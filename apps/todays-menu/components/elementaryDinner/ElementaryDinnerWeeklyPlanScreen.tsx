...
import {
  buildElementaryDinnerWeeklyShareCardModel,
  resolveElementaryDinnerSlotDisplay,
} from '../../services/weeklyPlan/elementaryDinnerWeeklyPlanDisplay';
import {
  captureElementaryDinnerShareCard,
  saveElementaryDinnerShareImage,
  shareElementaryDinnerShareImage,
} from '../../services/weeklyPlan/elementaryDinnerWeeklyPlanShare';
import {
  createElementaryDinnerWeekSeed,
  loadElementaryDinnerWeeklyPlanState,
  saveElementaryDinnerWeeklyPlanState,
...
export function ElementaryDinnerWeeklyPlanScreen() {
  ...
  const shareModel = useMemo(
    () => (plan ? buildElementaryDinnerWeeklyShareCardModel(plan) : null),
    [plan],
  );
  ...
  const applyPlan = useCallback(async (next: WeeklyMealPlan, seed: string, trackRefresh: boolean) => {
    setPlan(next);
    setStatus('ready');
    await saveElementaryDinnerWeeklyPlanState({ seed, plan: next });
    if (trackRefresh) {
      trackElementaryWeeklyPlanRefresh({ mode: 'dinner', seed });
    }
  }, []);

  const generateAndApply = useCallback(
    async (seed: string, trackRefresh: boolean): Promise<boolean> => {
      const result = generateElementaryDinnerWeek(seed);
  ...
  const loadInitial = useCallback(async () => {
    setStatus('loading');
    try {
      const stored = await loadElementaryDinnerWeeklyPlanState();
      if (stored) {
        await applyPlan(stored.plan, stored.seed, false);
        return;
      }
      const seed = createElementaryDinnerWeekSeed();
  ...
  useEffect(() => {
    ...
    trackElementaryWeeklyPlanView({ mode: 'dinner', seed: plan.seed });
  }, [plan?.seed, status]);

  const captureCurrentCard = useCallback(async (): Promise<string | null> => {
    try {
      await waitForNextFrame();
      return await captureElementaryDinnerShareCard(shareCardRef);
    } catch {
  ...
  const handleSaveImage = useCallback(async () => {
    ...
    try {
      ...
      if (!uri) return;
      const result = await saveElementaryDinnerShareImage(uri);
      if (result === 'ok') {
        trackElementaryWeeklyPlanImageSave({ mode: 'dinner', seed: plan.seed });
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      const uri = await captureCurrentCard();
      if (!uri) return;
      const result = await shareElementaryDinnerShareImage(uri);
      if (result === 'ok') {
        trackElementaryWeeklyPlanShare({ mode: 'dinner', seed: plan.seed });
        return;
      }
      if (result === 'cancelled') return;
        ...
        >
          <View style={styles.frame}>
            ...
            <ElementaryWeeklyMealSwitch
              ...
              onSelectBreakfast={() => router.replace(ELEMENTARY_BREAKFAST_WEEK_HREF)}
              onSelectDinner={() => {}}
            />
            ...
            {status === 'loading' ? (
              ...
              <View style={styles.list}>
                {plan.slots.map((slot) => {
                  const display = resolveElementaryDinnerSlotDisplay(slot);
                  return (
