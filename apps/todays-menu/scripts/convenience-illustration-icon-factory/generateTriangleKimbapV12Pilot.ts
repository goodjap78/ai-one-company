/**
 * Sprint 56-C.3 — triangle_kimbap v1.2 review PNG (center vertical seaweed band).
 */
...
import { writeTriangleKimbapV12ReviewHtml } from './writeReviewHtml';
...
export async function generateTriangleKimbapV12Pilot(): Promise<{
  ok: boolean;
  triangleKimbapV12Path?: string;
  error?: string;
}> {
  if (TRIANGLE_KIMBAP_V12_GENERATION.approvedSprint !== '56-C.3') {
    return { ok: false, error: 'triangle_kimbap v1.2 generation not approved for this sprint' };
  }

  if (fs.existsSync(PATHS.triangleKimbapMaster)) {
    return {
      ok: false,
      error: 'triangle_kimbap production master exists — rollback required',
    };
  }
  ...
  const outAbs = path.join(PATHS.reviewDir, TRIANGLE_KIMBAP_V12_FILE);
  if (fs.existsSync(outAbs)) {
    return {
      ok: false,
      error: `${TRIANGLE_KIMBAP_V12_FILE} already exists — delete manually to regenerate`,
    };
  }
  ...
  const prompt = buildTriangleKimbapV12Prompt(profile);

  fs.mkdirSync(PATHS.generatedRoot, { recursive: true });
  fs.mkdirSync(path.join(PATHS.generatedRoot, 'prompts'), { recursive: true });
  fs.writeFileSync(
    PATHS.triangleKimbapV12Prompt,
    `# triangle_kimbap v1.2 pilot prompt (Sprint 56-C.3)\n\n${prompt}\n`,
    'utf8',
  );
  ...
  const assetKey = TRIANGLE_KIMBAP_V12_GENERATION.allowedAssetKey;
  if (assetKey !== `${TRIANGLE_KIMBAP_PILOT_ICON_KEY}_v12`) {
    return { ok: false, error: `asset key gate failed: ${assetKey}` };
  }

  const provider = createImageProvider(env);
  console.log(`Generating ${TRIANGLE_KIMBAP_V12_FILE} via ${provider.name} (Sprint 56-C.3)…`);
  ...
  const scaled = scaleReviewPngToBbox(
    outAbs,
    'triangle_kimbap_v12',
    TRIANGLE_KIMBAP_V12_GENERATION.targetBboxMinPct,
    TRIANGLE_KIMBAP_V12_GENERATION.targetBboxMaxPct,
  );
  console.log(`Scaled triangle_kimbap v1.2 ${scaled.linearScale}x → bbox ${scaled.bboxPct}%`);

  writeTriangleKimbapV12ReviewHtml({ profile });
  ...
  const v12M = analyzeIngredientPng(outAbs, 'triangle_kimbap_v12');
  console.log(
    `ramen bbox ${(ramenM.bboxAreaRatio * 100).toFixed(1)}% · rice bbox ${(riceM.bboxAreaRatio * 100).toFixed(1)}% · v1.2 bbox ${(v12M.bboxAreaRatio * 100).toFixed(1)}%`,
  );
  console.log(`Review PNG: ${outAbs}`);
  console.log(`Review HTML: ${PATHS.reviewIndex}`);

  return { ok: true, triangleKimbapV12Path: outAbs };
}
