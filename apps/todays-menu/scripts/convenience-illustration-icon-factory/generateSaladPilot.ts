/**
 * Sprint 56-E — salad v1 pilot review PNG (Style Lock v1.0).
 */
...
import { SALAD_NEXT_PILOT, SALAD_PILOT_GENERATION } from './convenienceIconStyleLock';
import {
  SALAD_PILOT_ICON_KEY,
  SALAD_V1_FILE,
  PATHS,
  REFERENCE_INGREDIENT_KEYS,
} from './config';
import { writeSaladPilotReviewHtml } from './writeReviewHtml';
...
export async function generateSaladPilot(): Promise<{
  ok: boolean;
  saladPath?: string;
  error?: string;
}> {
  if (SALAD_PILOT_GENERATION.approvedSprint !== '56-E') {
    return { ok: false, error: 'salad generation not approved for this sprint' };
  }
  ...
  const outAbs = path.join(PATHS.reviewDir, SALAD_V1_FILE);
  if (fs.existsSync(outAbs)) {
    return {
      ok: false,
      error: `${SALAD_V1_FILE} already exists — delete manually to regenerate`,
    };
  }
  ...
  fs.writeFileSync(
    PATHS.saladV1Prompt,
    `# salad v1 pilot prompt (Sprint 56-E)\n\n${prompt}\n`,
    'utf8',
  );
  ...
  const assetKey = SALAD_PILOT_GENERATION.allowedAssetKey;
  if (assetKey !== `${SALAD_PILOT_ICON_KEY}_v1`) {
    return { ok: false, error: `asset key gate failed: ${assetKey}` };
  }

  const provider = createImageProvider(env);
  console.log(`Generating ${SALAD_V1_FILE} via ${provider.name} (Sprint 56-E only)…`);

  const result = await provider.generateImage({
    assetKey,
    subject: SALAD_NEXT_PILOT.koreanName,
    prompt,
  ...
  if (sha256File(PATHS.milkMaster) !== milkHashBefore) {
    return { ok: false, error: 'milk master changed during generation' };
  }

  const scaled = scaleReviewPngToBbox(
    outAbs,
    'salad_v1',
    SALAD_PILOT_GENERATION.targetBboxMinPct,
    SALAD_PILOT_GENERATION.targetBboxMaxPct,
  );
  console.log(`Scaled salad ${scaled.linearScale}x → bbox ${scaled.bboxPct}%`);

  writeSaladPilotReviewHtml({ profile });
  ...
  const milkM = analyzeIngredientPng(PATHS.milkMaster, 'milk_master');
  const saladM = analyzeIngredientPng(outAbs, 'salad_v1');
  console.log(
    `ramen bbox ${(ramenM.bboxAreaRatio * 100).toFixed(1)}% · rice bbox ${(riceM.bboxAreaRatio * 100).toFixed(1)}% · triangle bbox ${(kimM.bboxAreaRatio * 100).toFixed(1)}% · milk bbox ${(milkM.bboxAreaRatio * 100).toFixed(1)}% · salad bbox ${(saladM.bboxAreaRatio * 100).toFixed(1)}%`,
  );
  console.log(`Review PNG: ${outAbs}`);
  console.log(`Review HTML: ${PATHS.reviewIndex}`);

  return { ok: true, saladPath: outAbs };
}
