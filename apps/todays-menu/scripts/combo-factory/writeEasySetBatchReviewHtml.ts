/**
 * Sprint 53 — EASY_SET batch review HTML.
 */
import fs from 'node:fs';
import path from 'node:path';
import { loadEasySetComboAuditJson, type ComboAuditRow } from './auditComboHeroV2';
import { loadComboQueue } from './buildQueue';
import { PATHS } from './config';
import { easySetBatchComboIds, type EasySetBatchNumber } from './easySetBatchScope';
...
function productionExists(imageKey: string): boolean {
  const abs = path.join(PATHS.comboAssetsDir, `${imageKey}.jpg`);
...
function cardHtml(item: ComboQueueItem, audit?: ComboAuditRow): string {
  ...
  const itemsList =
    item.items.length > 0
      ? `<p class="items"><strong>items:</strong> ${escapeHtml(item.items.join(', '))}</p>`
      : '';
  ...
  const prod = productionExists(item.imageKey);
  ...
  return `<article class="card" data-status="${escapeHtml(item.status)}">
    ...
    <div class="meta">
      ...
      ${itemsList}
      <p class="production">Production: <strong>${prod ? 'yes' : 'no'}</strong></p>
...
export function writeEasySetBatchReviewHtml(batch: EasySetBatchNumber): string {
  fs.mkdirSync(PATHS.reviewDir, { recursive: true });
  const comboIds = easySetBatchComboIds(batch);
  const queue = loadComboQueue();
  const auditRows = loadEasySetComboAuditJson(batch) ?? [];
  ...
  const fileName = `easy-set-batch-${batch}.html`;
...
<head>
  ...
  <title>HANKKI EASY_SET Batch ${batch} Review</title>
  <style>
    :root { --bg:#fff4ec; --card:#fff; --ink:#2b2118; --muted:#7a6a5c; --accent:#27ae60; --border:#edd9c8; }
    * { box-sizing: border-box; }
    ...
    .ids,.items,.production { margin:0 0 8px; color:var(--muted); font-size:0.85rem; }
...
<body>
  <header>
    <h1>EASY_SET Batch ${batch} — Convenience EASY_SET Hero v2.0</h1>
    <p class="summary">
      Style: HANKKI Convenience EASY_SET Hero Style v2.0 · 1344×768 JPG · 82–90% framing · separate set items<br />
