/**
 * Sync recipeStepImageAssets.ts from on-disk assets/recipe-steps/*.jpg
 * Run: npx tsx scripts/sync-recipe-step-image-registry.ts
 */
...
const TARGET = path.join(APP_ROOT, 'services', 'images', 'recipeStepImageAssets.ts');
...
function main(): void {
  ...
  const onDisk = fs
    ...
    .filter((f) => /^[a-z0-9_]+\.jpg$/i.test(f))
    .map((f) => f.replace(/\.jpg$/i, ''))
    .sort((a, b) => a.localeCompare(b));

  const allKeys = [...new Set([...LEGACY_KEYS, ...onDisk])].sort((a, b) =>
    a.localeCompare(b),
  );

  const union = allKeys.map((k) => `  | '${k}'`).join('\n');
  const requires = onDisk
    .map((k) => `  ${k}: require('../../assets/recipe-steps/${k}.jpg'),`)
    .join('\n');
...
`;
  ...
  console.log(`onDisk registered: ${onDisk.length}`);
  console.log(`union keys: ${allKeys.length}`);
}
